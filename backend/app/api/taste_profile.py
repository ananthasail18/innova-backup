...
def submit_quiz_and_generate_profile(submission: QuizSubmission, db: Session = Depends(get_db)):
    # 1. Map quiz answers to a TasteProfileCreate schema using the Service
    profile_create = TasteIdentityService.generate_profile(submission)
    
    # 2. Calculate a basic confidence score (based on answering all questions, e.g., 8)
    # Simple logic: cap at 0.9 confidence for now
